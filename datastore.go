...
// A DatastoreLayer represents a cache on top of AppEngine's datastore.
// It implements Cache and GCable interfaces.
type DatastoreLayer struct{}

// Set sets a key to value with a given expiration.
func (d DatastoreLayer) Set(c appengine.Context, key string, value []byte, expiration time.Duration) error {
	return set(d, c, key, value, expiration)
}

// Get gets a value given a key.
func (d DatastoreLayer) Get(c appengine.Context, key string) ([]byte, error) {
	return get(d, c, key)
}

// SetItem sets a key to item.
func (d DatastoreLayer) SetItem(c appengine.Context, key string, item Item) error {
...
func (d DatastoreLayer) GetItem(c appengine.Context, key string) (Item, error) {
	k := datastore.NewKey(c, "Item", "Cache:"+key, 0, nil)
	item := Item{}
...
func (d DatastoreLayer) Delete(c appengine.Context, key string) error {
	k := datastore.NewKey(c, "Item", "Cache:"+key, 0, nil)
	return datastore.Delete(c, k)
}

// Flush removes all cache items from the datastore.
func (d DatastoreLayer) Flush(c appengine.Context) error {
...
func (d DatastoreLayer) GC(c appengine.Context) error {
